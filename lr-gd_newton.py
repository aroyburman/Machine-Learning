...
def newton(theta, x, y, alpha, tol, maxiter):
    nll_vec = []
    nll_vec.append(neg_log_like(theta, x, y))
    nll_delta = 2.0*tol
    iter = 0
    while (nll_delta > tol) and (iter < maxiter):
        theta = theta - np.dot(np.linalg.inv(hess(theta,x)),log_grad(theta, x, y)) 
        nll_vec.append(neg_log_like(theta, x, y))
...
# Run newton method
alpha = 1e-6 #.000000000000001
tol = 1e-3
maxiter = 10000
import time
start = time.time()
theta,cost = newton(theta,xtilde,y, alpha, tol,maxiter)
end = time.time()

print ('Running time  of: ' + str(end-start))
print ('Final value of negative log-likelihood: ' + str(cost[-1]))
print ('Number of iterations: ' + str(len(cost)-1))
